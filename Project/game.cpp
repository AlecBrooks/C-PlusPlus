...
class Hero{
	...
	Hero(string name, int maxHP, int hp, int st, int dex, Weapon* weapon, Room* room)
    : name(name), maxHP(maxHP), hp(hp), st(st), dex(dex), armorClass(10 + dex), weapon(weapon), room(room){}
...
int main(){
	...
	//weapons
	Weapon starterSword("Rusty Sword", 2, 5);
	Weapon claw("Bloodly Claws", 2, 3);
	
	//monsters
	Monster goblin("Goblin", 10, 1, 1, &claw, "A small, green creature with bloodstained claws and a cruel grin.", "The goblin screeches as your blow lands true. Blood sprays from its mouth as it stumbles backward, eyes wide in shock. With a final gasp, it crumples to the ground, twitching once before going still.");

	//player
	Hero player("Aragorn", 30, 30, 3, 2, &starterSword, &testRoom);
